...
> Draft v2 (2026-07-09). Integrates the v1 text (folder 89), Table 1–3 bodies (folder 90), and the
> canonical figure set with captions (folder 91). Citation placeholders remain pending the folder-86
> audit and are marked as `[..._REF]`.
...
|---|---:|---:|---|---:|---:|---|
| Radar frequency agility / glint reduction [RADAR_FREQ_AGILITY_REF] | Yes | No | Decorrelate radar angular/glint tracking errors | Partly analogous, but not underwater USBL multipath | No | We cite this as the closest physical analogy, then narrow the claim to shallow-water USBL and surface-reflection-induced coherent DOA residuals. |
| Frequency-hopped acoustic modem USBL [FH_USBL_REF] | Yes | Yes | Use frequency-hopped acoustic modem sequences for positioning | Not established as the target mechanism | No | Prior existence prevents a "first FH USBL" claim; our contribution is the residual-whitening mechanism and validation boundary. |
| Costas / hopped USBL shallow-water designs [COSTAS_USBL_REF] | Yes | Yes | Improve correlation, time-delay, or navigation precision | Not the stated mechanism | No | Related waveform diversity exists, but does not isolate carrier-locked post-gating DOA bias entering a UKF tracker. |
| Acoustic frequency-comb / iUSBL approaches [FREQ_COMB_REF] | Yes | Often iUSBL or related positioning | Use comb/frequency-diverse signals for positioning or processing | Not the stated mechanism | No | Frequency diversity is used, but the system role and residual mechanism differ. |
...
|---|---|---|---|---|
| Exact citation metadata still pending for several placeholders | Audit protocol defined but not all references closed | Reviewer may challenge novelty framing | Verify publisher metadata for radar glint/frequency agility, JASA FH USBL, IEEE Costas USBL, frequency-comb/iUSBL | Before submission; Related Work |
...
## Figure File Manifest (canonical, from folder 91)

| Callout | File (in `91. Figure file alignment/figures/`) | Note |
|---|---|---|
| Fig. 1 | `fig1_system_concept.svg` / `.png` | concept draft; final visual polish pending |
...
| Fig. 5 | `fig5_quasi_static_speed_boundary.svg` | PNG not yet generated (convert if Word workflow needs raster) |
| Fig. 6 | `fig6_crlb_floor.svg` / `.png` | canonical name; supersedes legacy `fig7_crlb_floor.*` |

## References (placeholders — folder-86 audit pending)

- `[RADAR_FREQ_AGILITY_REF]` — radar frequency agility / glint decorrelation classic(s). Existence confirmed on IEEE Xplore (documents 5408996, 4101322); exact author/year metadata to be finalized via library access. Public fallback: Nathanson, "Frequency Agility for Radar Target Detection," Johns Hopkins APL Technical Digest.
...
- `[COSTAS_USBL_REF]` — "Optimizing baseline in USBL using Costas hopping to increase navigation precision in shallow water," IEEE IMCOM 2022 (IEEE Xplore 9721736). Full-text differentiation check pending.
- `[FREQ_COMB_REF]` — "A Passive Inverted Ultrashort Baseline Scheme for Underwater Positioning Based on the Acoustic Frequency Combs," IEEE/OES China Ocean Acoustics (COA) 2024.
