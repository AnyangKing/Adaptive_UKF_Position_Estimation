...
이번 질문:
  ...
  65번 pilot의 이득이 독립 seed 중간규모 검증에서도 재현되는가?
...
def run():
    ...
    partial_path = out / "anchor_hop_midscale_partial.json"
    ...
    payload = {
        "config": {
            ...
            "note": "65번 후속 독립 seed 중간규모 검증. fixed carrier anchor와 sparse hop probe schedule 비교.",
        },
        "summary": summarize(rows),
        "runs": rows,
    }
    (out / "anchor_hop_midscale.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
