"""Independent moving validation for carrier-transition-aware Adaptive-R.

This script freezes the rule discovered in folder 180 and tests it on new
moving-target seeds.  Ground truth is used only for signal synthesis and final
error computation.
"""
...
def run(max_workers: int = 6) -> dict[str, Any]:
    ...
    criteria = {
        ...
        "wilcoxon_p_lt_0_05_independent": cmp["wilcoxon_gain_gt0_p"] < 0.05,
        ...
        "mean_gain_vs_fixed_positive": summary["comparisons"]["softR_vs_fixed"]["mean_gain_m"] > 0.0,
    }
    payload = {
        "config": {
            ...
            "manuscript_claim_allowed": "conditional_after_claim_audit",
        },
        "summary": summary,
        "criteria": criteria,
        "decision": "validated_independent_moving_candidate" if all(criteria.values()) else "not_validated_independent_moving",
        "trials": rows,
    }
    out = HERE / "transition_aware_independent_moving_validation.json"
