# 167 caption correction changes

167번 감사 이후 리뷰어 관점에서 지적된 3건을 167번 폴더 안에서 마무리한다.
...
## 2. `caption_changes.md` 문서 정합성 보정
...
따라서 167번 폴더 안에서 다음 표현으로 정합화했다.
```
원고 본문 또는 원천 실험 폴더(61, 63, 82, 45, 58, 138, 145)에 이미 있던 수치만 재확인.
```

이유:
- 원고 claim boundary를 넘는 새 수치 창출은 없지만, "본문에 이미 존재"보다는 "본문 또는 원천 실험
  폴더에 존재"가 더 정확하다.

## 3. `README.md` "6원칙" 표기 보정

167 폴더 `README.md` line 14는 "캡션 6원칙"이지만, `caption_policy.md`에는 7개 항목이 있다.
정책 문서와 요약 문서의 항목 수가 맞도록 167번 README를 다음 표현으로 보정했다.
```
...
- 기록 정합화 대상 파일 3개: `README.md`, `caption_changes.md`, `caption_correction_changes.md`.
