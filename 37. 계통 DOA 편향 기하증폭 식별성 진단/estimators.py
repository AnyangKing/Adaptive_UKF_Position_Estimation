...
def estimate_toa_tracked(
    received: np.ndarray,
    cfg: ChannelConfig,
    previous_toas_s: np.ndarray | None,
    maximum_jump_s: float = 0.002,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """이전 TOA 근처의 유효 peak를 선택해 경로 switching을 억제한다."""
    probe = make_probe(cfg)
    toas=np.empty(received.shape[0]); qualities=np.empty(received.shape[0]); fallback=np.zeros(received.shape[0],dtype=bool)
    minimum_spacing=max(1,len(probe)//4)
    for i,signal in enumerate(received):
        correlation=correlate(signal,probe,mode="full",method="fft")
        lags=correlation_lags(len(signal),len(probe),mode="full")
        valid=lags>=0; envelope=np.abs(correlation[valid]); valid_lags=lags[valid]
        peaks,_=find_peaks(envelope,height=0.12*np.max(envelope),distance=minimum_spacing)
        if len(peaks)==0:
            peaks=np.array([int(np.argmax(envelope))])
        candidate_times=valid_lags[peaks]/cfg.sample_rate_hz
        if previous_toas_s is None:
            peak=int(peaks[np.argmax(envelope[peaks])])
        else:
            differences=np.abs(candidate_times-previous_toas_s[i])
            nearby=np.where(differences<=maximum_jump_s)[0]
            if len(nearby):
                # 가장 가까운 후보가 여러 sidelobe 중 물리적으로 연속인 경로를 유지한다.
                chosen=nearby[np.argmin(differences[nearby])]
                peak=int(peaks[chosen])
            else:
                peak=int(peaks[np.argmax(envelope[peaks])]); fallback[i]=True
        fractional=_parabolic_peak(envelope,peak)
        toas[i]=(valid_lags[0]+fractional)/cfg.sample_rate_hz
        qualities[i]=envelope[peak]/(np.median(envelope)+1e-12)
    return toas,qualities,fallback
...
def pair_delays_to_reference_tdoa(pair_delays_s: np.ndarray, sensor_count: int = 8) -> np.ndarray:
    """28개 pair delay를 일관된 센서 지연으로 LS 복원하고 sensor 0 기준값 7개를 반환한다."""
    pair_delays = np.asarray(pair_delays_s, dtype=np.float64)
    expected = sensor_count * (sensor_count - 1) // 2
    if pair_delays.shape != (expected,):
        raise ValueError(f"expected {expected} pair delays")
    # q0=0으로 고정하고 q_i-q_j=d_ij를 최소제곱한다.
    matrix, values = [], []
    index = 0
    for i in range(sensor_count):
        for j in range(i + 1, sensor_count):
            row = np.zeros(sensor_count - 1)
            if i > 0:
                row[i - 1] += 1.0
            if j > 0:
                row[j - 1] -= 1.0
            matrix.append(row)
            values.append(pair_delays[index])
            index += 1
    relative, *_ = np.linalg.lstsq(np.asarray(matrix), np.asarray(values), rcond=None)
    return relative
