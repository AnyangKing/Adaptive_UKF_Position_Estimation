...
def paths_for_sensor(source_m: np.ndarray, sensor_m: np.ndarray, cfg: ChannelConfig) -> list[Path]:
    """직접파, 평면 해수면·해저 반사의 image-source 경로. 옵션으로 2차 반사 포함."""
    source = np.asarray(source_m, dtype=np.float64)
    H = cfg.water_depth_m
    surface_image = source.copy(); surface_image[2] = -source[2]
    bottom_image = source.copy(); bottom_image[2] = -2.0 * H - source[2]
    specifications = [
        ("direct", source, 1.0),
        ("surface", surface_image, cfg.surface_reflection),
        ("bottom", bottom_image, cfg.bottom_reflection),
    ]
    if cfg.second_order_multipath:
        # 표면→해저(surf_bot)와 해저→표면(bot_surf) 2차 반사 image source.
        surf_bot = source.copy(); surf_bot[2] = -2.0 * H + source[2]
        bot_surf = source.copy(); bot_surf[2] = 2.0 * H + source[2]
        specifications += [
            ("surf_bot", surf_bot, cfg.surface_reflection * cfg.bottom_reflection),
            ("bot_surf", bot_surf, cfg.bottom_reflection * cfg.surface_reflection),
        ]
    result = []
...
def synthesize_received(
    ...
    # 거친 표면 산란: 표면 반사경로마다 진폭·지연을 랜덤 섭동(재현 위해 seed 고정).
    rough_rng = np.random.default_rng(cfg.seed + 777) if cfg.surface_roughness > 0 else None
    # 굴절: 직접파를 굴절된 도착 고도각의 평면파로 대체(배열 작아 평면파 근사).
    direct_delays = None
    if cfg.sound_speed_gradient != 0.0:
        from refraction import refraction_elevation_shift
        center = sensors.mean(axis=0)
        delta = source - center
        az = np.arctan2(delta[1], delta[0])
        el = np.arctan2(delta[2], np.hypot(delta[0], delta[1]))
        el += refraction_elevation_shift(source, center, cfg)
        u = np.array([np.cos(el)*np.cos(az), np.cos(el)*np.sin(az), np.sin(el)])
        c_local = cfg.sound_speed_m_s + cfg.sound_speed_gradient * (-center[2])
        base = float(np.linalg.norm(source - center)) / cfg.sound_speed_m_s
        direct_delays = base - (sensors - center) @ u / c_local

    for sensor_index, paths in enumerate(all_paths):
        active = paths if include_multipath else paths[:1]
        for path in active:
            amplitude, delay = path.amplitude, path.delay_s
            if direct_delays is not None and path.name == "direct":
                delay = float(direct_delays[sensor_index])
            if rough_rng is not None and "surf" in path.name:
                amplitude *= max(0.0, 1.0 + rough_rng.normal(0.0, cfg.surface_roughness))
                delay += rough_rng.normal(0.0, cfg.surface_roughness / cfg.carrier_hz)
            warped_time = (time - delay) * (1.0 + doppler)
            received[sensor_index] += amplitude * np.interp(
