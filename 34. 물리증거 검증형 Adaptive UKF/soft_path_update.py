...
class CubaturePathMarginalizer:
    ...
    def evidence_score(self, mean, covariance, peak_times, peak_strengths):
        """상태 Gaussian 아래에서 경로배정을 주변화한 정규화 log evidence."""
        times=np.asarray(peak_times); strengths=np.asarray(peak_strengths)
        if len(times)<3: return float('-inf')
        points=_cubature_points(np.asarray(mean),np.asarray(covariance)); log_evidence=[]
        for point in points:
        ...
        return float(logsumexp(np.asarray(log_evidence))-np.log(len(points)))

    def update(self, ukf, peak_times, peak_strengths):
        times = np.asarray(peak_times); strengths = np.asarray(peak_strengths)
        if len(times) < 3:
            self.history.append({"applied": False}); return ukf.x.copy()
        points = _cubature_points(ukf.x, ukf.P); log_evidence = []
        for point in points:
        ...
        posterior_mean = weights @ points; difference = points - posterior_mean
        posterior_cov = np.einsum('i,ij,ik->jk', weights, difference, difference)
        # 균일 likelihood에서 cubature moment가 원래 P를 정확히 재현해야 한다.
        # retention은 ablation용이며 기본값 0이다.
        posterior_cov += self.covariance_retention * ukf.P + 1e-10 * np.eye(len(ukf.x))
