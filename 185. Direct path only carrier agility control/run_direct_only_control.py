"""Direct-path-only carrier-agility control experiment."""
...
def bootstrap_ci(values: np.ndarray, seed: int = 185, n: int = 5000) -> list[float]:
...
def run(max_workers: int = 6) -> dict[str, Any]:
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"completed direct-only {int(d)} m #{i}", flush=True)
    rows.sort(key=lambda r: (r["distance_m"], r["index"]))
    payload = {
        "config": {
            "stage": "mechanism_control",
            "control": "direct_path_only",
            ...
            "manuscript_claim_allowed": "mechanism_attribution_control_only",
        },
        "summary": summarize(rows),
        "trials": rows,
    }
    (HERE / "direct_only_control.json").write_text(
