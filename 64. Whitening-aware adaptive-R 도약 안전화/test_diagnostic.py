...
from run_whitening_guard import (
    CONDITIONS,
    DISTANCE,
    FIXED_CARRIER_HZ,
    GEOMS,
...
def test_pre_registered_conditions_and_policy_set():
    ...
    assert "hop_R4_jump1_x8" in POLICIES
    assert all(p["carrier"] in ("fixed", "hop") for p in POLICIES.values())


def test_hop_carriers_are_in_band_and_frozen():
    cfg = ChannelConfig()
    ...
    assert HOP_CARRIERS_HZ.max() + half_bw < cfg.sample_rate_hz / 2.0
...
def test_jump_gate_inflates_doa_R():
    cfg = ChannelConfig()
    ...
    wrapper = WhiteningAwareAdaptiveRUKF(ukf, base_doa_scale=1.0, jump_threshold_deg=0.1, jump_doa_scale=8.0)
    ...
    wrapper.step(z0, q)
    wrapper.step(z1, q)
    assert wrapper.history[-1]["jump_gate"]
    assert wrapper.history[-1]["R_doa_scale"] >= 8.0


if __name__ == "__main__":
    test_pre_registered_conditions_and_policy_set()
    test_hop_carriers_are_in_band_and_frozen()
    test_geometry_modes()
    test_lag1_autocorr_sanity()
    test_jump_gate_inflates_doa_R()
